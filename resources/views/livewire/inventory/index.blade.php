<div class="flex flex-col w-full mx-auto gap-8 p-8 xl:p-8 2xl:p-10 xl:gap-8 2xl:gap-10">
    ...
    <x-modal class="max-w-lg">
        <x-form title="inventory" :update="$update">

            <x-text-input label="Goods Name*" name="name" type="text" wire:model.defer="name" />
            <x-text-input label="Quantity" name="quantity" type="text" wire:model.defer="quantity" />
            <x-text-input label="Price*" name="price" type="text" wire:model.defer="price" />
            <div class="w-full h-12 space-y-1">
                <select wire:model.defer="category"
                    ...
                    @foreach (["drugs and medicine", "bed", "food"] as $option)
            ...
            <tbody class="w-full overflow-x-auto break-normal">
                ...
                <tr class="">
                    <td class="p-2 whitespace-nowrap">
                        <input type="checkbox" wire:model="checked" id="" value="{{ $inventory->id }}"
                    ...
                    <td class="p-2 text-black text-sm">
                        {!! DNS1D::getBarcodeHTML("$inventory->code", 'C39+', 1, 30, "BLACK") !!}
                        <span class="">{{ $inventory->code }}</span>
                    </td>
                    <td class="p-2 whitespace-nowrap">
                        <span class="py-1 px-2 rounded text-sm text-green-600 bg-green-100"> &#8358;
                            {{ moneyFormat($inventory->price) }}</span>
                    </td>
                    ...
                    {{-- <td class="p-2 whitespace-nowrap">
                    @if ($inventory->status == 'in-inventory')
                    <span
                        class="p-1 text-xs uppercase rounded bg-green-100 text-green-600 px-2 shadow">{{ $inventory->status }}</span>
                    @else
                    <span
                        class="p-1 text-xs uppercase rounded bg-red-100 text-red-600 px-2 shadow">{{ $inventory->status }}</span>
                    @endif
                    </td> --}}
                    <td class="p-2 whitespace-nowrap">
                        <div class="flex space-x-2 item-center">
                            <span wire:click="edit({{ $inventory->id }})"
                            ...
                            <span wire:click="confirmDelete({{ $inventory->id }})"
